...
| **MS3 Paper 上線** | 🟡 **建置完成但未持續運行(2026-08-02 更新)** | 對帳/告警/OMS/TCA/health/drift 已全建、機制本身正常;但**最後一筆實際交易決策停在 2026-07-18**,之後 07-20~24 與 07-27~08-01 **連續兩週、10 個交易日完全沒交易**。原因不是系統故障,是「靠人每天手動啟動」在實務上不成立(Claude 啟動的會隨 session 死;人會忘)。→ **「連跑 4 週、對帳零差異」的驗收時鐘尚未開始計時**,起點改為 24/7 雲端部署上線日(見 [docs/CLOUD_DEPLOY_ch.md](docs/CLOUD_DEPLOY_ch.md)) |
| **MS4 小額實盤** | 🟡 **前置已清,未開(2026-07-19 更新)** | 8 個 P0 **全數修復**(#1-#5/#7/#8 Batch 0、#6 Batch 1;#5 斷路器 2026-07-19 於三 spec 設值通電)。轉正式帳戶等 paper 4 週驗收 + 人為決定,**不自動發生** |

**一句話結論**:**研究層很紮實,但「真的拿去下單」的營運/安全層有致命缺口。**
最重要的一句:**在下面 P0 修好之前,請不要開 `--execute` 排程去自動下真單(即使是 paper 也會累積錯誤資料;轉正式帳戶更是不行)。**
> ✅ **2026-07-19 註**:上句的前提已解除——8 個 P0 全修(見下方 Batch 0/1 更新),paper `--execute` 已實跑;原文保留作歷史紀錄。正式帳戶(真錢)仍維持禁止,直到 paper 驗收通過且使用者明確拍板。
...
| **M3 供應鏈** | ❌ | — | 全缺 —— 目前非我們路線 |
| **M4 因子庫/研究框架** | 🟡 ~45% | journal 記 session、**實驗記錄系統(M4.5)**、**研究知識庫(M4.6:`research/notes.py` + `research_notes/`,`quant note new/list`,frontmatter 連實驗 id)** | **因子計算框架、因子檢定(IC/RankIC)** |
...
5. **live 熔斷修復** — 餵 P&L、`max_daily_loss` 進 LiveConfig、且熔斷放行減倉單(#5)。
   *(2026-07-19 補完最後一哩:三個 spec 的 risk 設 `max_daily_loss: 1000`(帳戶級,≈權益 1%)——程式早修好但 spec 沒設值=裝了沒通電;555.pdf 架構比對時發現並啟用。)*
