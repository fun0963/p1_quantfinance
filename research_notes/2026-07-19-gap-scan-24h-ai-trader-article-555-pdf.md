...
**真沒有、便宜、可選 → ✅ 2026-07-19 已做**:
- `quant watch --volume-spike MULT`(最後一根量 ≥ 前 N 根均量 × MULT;`--volume-window`
  預設 20,基準排除爆量 bar 自身)——無狀態一次性設計直接容納,idea generation 用途。
  實測:SPY 日線 x1.14 安靜;QQQ 1min 收盤分鐘 x3.24 觸發(收盤競價量能,條件真的有料)。
...
② ✅ watch 加 --volume-spike(2026-07-19 完成,見上);③ 其餘全部掛觸發條件緩議(見各段)。
