...
5. **接下來(依優先序)**:
   ...
   - ✅ **strategy_dev_kit 降級為呼叫配方(2026-07-26)**:外部沙盒的引擎副本(06-14 快照)整批移除——副本會漂移、數字不可引用;README 重寫為「從 p1 呼叫 `quant … --json` / 唯讀 MCP」的配方 + 紀律四條(沙盒數字僅初篩、發想前查 rejected 筆記、不做跨標的挖礦、轉正走 p1 關卡)。沙盒=亂想層,判決唯一在 p1。
...
6. **多消費者設計備忘(2026-07-26 拍板)**——三種資料、三種歸屬,混雜只發生在歸屬切錯時:
   ...
   - **「台股到來」清單**:shioaji 券商類(01_ref 有參考)、XTAI 交易日曆進排程/新鮮度閘、賣出 0.3% 證交稅進成本模型、TWD 幣別進 metrics/journal、`MARKET=tw` 的 feed 註冊。
