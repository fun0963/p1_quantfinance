...
| **quant/data/** | 行情擷取 + 儲存 + 快取 + 品質 + 完整性 + **timeframe 註冊表** | `timeframes.py`(**Batch 4 單一真相來源**:bar 秒數/年化週期/vbt freq/預設 feed/intraday 旗標;未知 timeframe 一律 raise);`feeds/`(`DataFeed` ABC → `YFinanceFeed`/`AlpacaFeed` + `get_feed(timeframe)` 工廠:日級 yfinance、盤中 Alpaca);`storage/`(`BarStore` ABC → `ParquetStore`/`TimescaleStore` + `get_store()` 工廠;**parquet 版面 `data/bars/{market}/`,market 預設 us 來自 settings——市場命名空間,見多消費者備忘**);`loaders.py`:`load_bars()`(快取+新鮮度[日級 days / 盤中 bars]+品質+改寫偵測)、`fetch_bars()`;`quality.py`;`integrity.py`(point-in-time 護欄) | core、config | `test_storage/test_loaders/test_quality/test_integrity/test_timeframes/test_intraday` |
...
5. **接下來(依優先序)**:
   ...
   - ⬜ **開盤日記得啟動(只能使用者親手)**:交易日開機後雙擊 `scripts\trading.cmd`,用 `quant status` 一眼確認。**不要叫 Claude 代啟動**——Claude 啟動的行程會隨 session 收攤而死(見 §9 陷阱,三次實證);Claude 的角色是查核回報。07-20~07-24 因此中斷過一整週,4 週 paper 驗收時鐘自 07-27 重計。
   ...
   - ✅ **工具組統一、strategy_dev_kit 刪除(2026-07-26)**:外部引擎副本(06-14 快照)**整個資料夾移除**——副本會漂移、數字不可引用。呼叫配方移進 p1:[docs/SANDBOX_AGENT_ch.md](docs/SANDBOX_AGENT_ch.md)(呼叫指令、結果去哪看、紀律四條)。**真正的研究沙盒是 `D:\AI_work_claude\p3_strategy`**(台股日線波段,S0~S9 淘汰漏斗);它原本指向 dev_kit 的 8 處引用已一併修正指向 p1,並標注「台股行為層尚未建置」。→ 連帶發現:多消費者不再是假設,見待辦「台股行為層評估」。
   - ✅ **多消費者命名空間(2026-07-26)**:bars 快取改 `data/bars/{market}/`(settings.market 預設 us,一次性搬移完成、快取原地服務驗證過);journal 路徑可用 `JOURNAL_DB` 覆寫(慣例:一帳戶一 DB);行為層(shioaji/日曆/稅/幣別)刻意不建,見下方「多消費者設計備忘」。
   - ✅ **24/7 雲端部署就緒(2026-08-02,使用者拍板採 Oracle Cloud Always Free)**:①**單寫者閘** `EXECUTE_HOST`——同帳戶兩個 `--execute` 排程器會下兩倍部位,故 CLI 在 hostname 不符時拒絕下單(雲端容器固定 `quant-live`,本機 .env 設同值即自動禁用本機下單;未設=不限制,向後相容);②`Dockerfile.live` 輕量映像——**live 路徑完全不 import vectorbt/backtrader**(AST 測試 + 封鎖 import 的動態實測雙重釘死),砍掉兩個最重且最不 ARM 友善的 wheel,免費層跑得動;③`docker-compose.live.yml` 兩個 service(`--every`/`--fraction` 是行程級參數,日線與 5min 不能共存於一行程)+ `restart: unless-stopped` + volume 持久化;④`tests/test_deploy.py` 防漂移(引擎 import、依賴清單與 pyproject 同步、compose 的 hostname/EXECUTE_HOST 一致);⑤[docs/CLOUD_DEPLOY_ch.md](docs/CLOUD_DEPLOY_ch.md) 完整手冊(含 Oracle ARM 缺貨與**閒置回收**風險、儀表盤禁止公網曝露、三層監控、遷移檢查清單)。**VM 建立與金鑰上傳由使用者執行**。
...
6. **多消費者設計備忘(2026-07-26 拍板)**——三種資料、三種歸屬,混雜只發生在歸屬切錯時:
   - **市場資料按「市場」分**:`data/bars/us/`、(未來)`data/bars/tw/`——帳戶間共用;台股代號用本地碼(2330),「.TW」後綴是 yfinance 的方言,由 feed 轉接層翻譯,快取檔名保持乾淨。
   - **營運狀態按「帳戶」分**:一個券商帳戶 ↔ 一本 journal.db(`JOURNAL_DB` 指路)——這是對帳的結構性不變量;spec 不含帳戶(如同不含 execute),綁哪個帳戶是啟動器層的事;帳戶級斷路器 `day_pnl()` 在多帳戶下天然各管各的。
   - **研究知識全域一份**:notes/experiments/spec——墳場只有一座,實驗是引擎的證詞,引擎共用證詞就共用。
   - **「第二帳戶到來」清單**:環境 profile(第二組券商金鑰)、`JOURNAL_DB` 指到新檔、trading.ps1 jobs 分帳戶分組、各帳戶自訂 max_daily_loss 值。
   - **「台股到來」清單**:shioaji 券商類(01_ref 有參考)、XTAI 交易日曆進排程/新鮮度閘、賣出 0.3% 證交稅進成本模型、TWD 幣別進 metrics/journal、`MARKET=tw` 的 feed 註冊。**⚠️ 2026-07-26 起這不再是假設**:`p3_strategy`(台股 S0~S9 研究專案)是真實在等的消費者,目前卡在 S1~S3 補課、尚未需要回測工具;S4 之後會撞上這道牆,屆時決定「先建台股層」或「先用美股標的驗證方法論」。
   - **引擎抽成獨立套件的觸發**(維持緩議):策略數多到研究噪音淹沒 CI、或出現第二個消費者要共用引擎、或多人協作——屆時資料歸屬已乾淨,搬的只剩程式碼。
7. **範圍外(暫不做)**:台股、選擇權、M2 公司庫、M3 供應鏈、M9 事件層。
...
- **⚠️ 由 Claude 啟動的常駐程序在 session 收攤時必死——不要再試第三種逃逸**:兩次實驗都陣亡。①Start-Process 批(07-18、07-19):log 乾淨結束在啟動行、無 crash、無 reboot,**07-20~07-24 整週沒交易**;②WMI 批(07-26):改 `Invoke-CimMethod Win32_Process.Create`(WmiPrvSE 代生、已驗證脫離呼叫者 job),啟動 10 分鐘後仍隨 Claude 行程退出而死(heartbeat 止於 05:49,事件檔證明機器醒著、無關機無重開)——**清理範圍是沙箱層級,比 Job Object 廣**。附帶教訓:uptime/LastBootUpTime 在 Windows 快速啟動下會騙人,判斷「有沒有重開機」要查事件檔 6005/6006/1074。**正解:交易排程器由使用者雙擊 trading.cmd 啟動**(explorer 父行程、真使用者 session,活到關機為止=設計本意);Claude 只做查核回報。WMI 生成改動保留(對使用者雙擊無害,且環境變數不繼承的 PYTHONPATH 內嵌是通用正確性修復)。
- **⚠️ 依賴全是「>= 下限、無上限」,CI 會被上游改版打紅**:CI 每次都裝最新版,所以破壞性改版會在你什麼都沒改的情況下讓 CI 變紅。**實例(2026-08-02)**:`mcp` 發布 2.0,移除了 `mcp.server.fastmcp` 路徑,`mcp_server.py` 直接 ImportError;本機仍是 1.28.1 所以完全綠。處置:pin `mcp>=1.2,<2` 恢復綠燈,把 2.x 遷移當獨立工作(不在修紅燈時混改 API)。**診斷法**:`gh run view <id> --log | grep "Successfully installed"` 看 CI 實際裝了什麼版本,和本機 `pip show` 比對。其他浮動依賴(vectorbt 已到 1.0、numpy 2.4、pytest 9)同樣有此風險。
