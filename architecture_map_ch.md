...
5. **接下來(依優先序)**:
   ...
   - ✅ **多消費者命名空間(2026-07-19)**:bars 快取改 `data/bars/{market}/`(settings.market 預設 us,一次性搬移完成、快取原地服務驗證過);journal 路徑可用 `JOURNAL_DB` 覆寫(慣例:一帳戶一 DB);行為層(shioaji/日曆/稅/幣別)刻意不建,見下方「多消費者設計備忘」。
...
6. **多消費者設計備忘(2026-07-19 拍板)**——三種資料、三種歸屬,混雜只發生在歸屬切錯時:
