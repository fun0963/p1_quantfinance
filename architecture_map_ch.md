...
- **⚠️ AI 工具 shell 用 Start-Process 啟動的常駐程序會無聲死亡**:排程器掛在該 shell 的 Windows Job Object 上,session 行程樹清理時整批被帶走——log 乾淨結束在啟動行、無 crash、無 reboot(實錘:uptime 10.7 天但 07-18 與 07-19 兩批排程器都消失,**07-20~07-24 整週沒交易**)。使用者雙擊 trading.cmd(explorer 父行程)不受影響。修法(2026-07-26):trading.ps1 改 `Invoke-CimMethod Win32_Process.Create`(WmiPrvSE 代生,脫離呼叫者 job;`ShowWindow=[uint16]0` 隱藏視窗;**環境變數不繼承,PYTHONPATH 要烤進命令列**)。驗證:父行程=WmiPrvSE.exe。
